...
<div id="maindiv" style="position:absolute; width:100%">
	<?php
	    for($i = 0; $i < sizeof($designers);$i=$i+3)
			...
			<a href="index.php?r=site/viewprofile&username=<?= $designers[$i]->designer_user_name ?>"><p><?= $name1->designer_first_name." ".$name1->designer_last_name ?></p><p>Revenue <?= $name1->designer_revenue ?></p></a>
			</div>
			<?php } ?>
			<?php if($todisplay2 == 'true') { ?>
			<div class="col-xs-3">
			<?= Html::img($imgpath2->profile_picture,['width'=>100,'height'=>100]);?><?= "<br/>" ?>
			<a href="index.php?r=site/viewprofile&username=<?= $designers[$i+1]->designer_user_name ?>"><p><?= $name2->designer_first_name." ".$name2->designer_last_name ?></p><p>Revenue <?= $name2->designer_revenue ?></a>
			</div>
			<?php } ?>
			<?php if($todisplay3 == 'true') { ?>
			<div class="col-xs-3">
			<?= Html::img($imgpath2->profile_picture,['width'=>100,'height'=>100]);?><?= "<br/>" ?>
			<a href="index.php?r=site/viewprofile&username=<?= $designers[$i+2]->designer_user_name ?>"><p><?= $name3->designer_first_name." ".$name3->designer_last_name ?></p><p>Revenue <?= $name3->designer_revenue ?></a>
			</div>
			<?php } ?>
			</div>
		<?php echo "<br/>";}
